...
AlterLoader* AlterLoader::sharedAtlasLoader = nullptr;

AlterLoader* AlterLoader::getInstance(){
	if (sharedAtlasLoader == NULL) {
		sharedAtlasLoader = new AlterLoader();
	}
	return sharedAtlasLoader;
}


void AlterLoader::destroyInstance()
{
...
void AlterLoader::loadAtlas(string filename){
	auto textureAtlas = Director::getInstance()->getTextureCache()->addImage("atlas.png");
	this->loadAtlas(filename, textureAtlas);
}

void AlterLoader::loadAtlas(string filename, Texture2D *texture) {
	/*string data = FileUtils::getInstance()->getStringFromFile(filename);
	CCSpriteFrameCache::getInstance()->addSpriteFramesWithFileContent(data, texture);*/
	string data = FileUtils::getInstance()->getStringFromFile(filename);
...
SpriteFrame* AlterLoader::getSpriteFrameByName(string name){
